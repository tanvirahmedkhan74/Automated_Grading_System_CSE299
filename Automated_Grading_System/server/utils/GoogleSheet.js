...
const criteria_based_grading = async (studentResponses, quest) => {
    const BASE_API_URL = 'http://127.0.0.1:5000';
    const response = [];

    const questions = [];

    // Cleaning the first row of the sheet
    for (const [questionIndex, question] of quest.entries()) {
      if(question.question !== 'Questions[MAX_MARK]'){
